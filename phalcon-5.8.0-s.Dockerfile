...
ZEPHIR_PARSER_VERSION=1.6.1 IGBINARY_VERSION=3.2.15 PSR_VERSION=1.2.0 MSGPACK_VERSION=2.2.0 EVENT_VERSION=3.1.3 EV_VERSION=1.1.5
...
RUN apk add --no-cache --virtual build-dependencies build-base tzdata autoconf && \
    ...
    /usr/bin/install-php-extensions inotify swoole && \
...
# 注意：需要手动启用 xdebug（无法查看到 xdebug.ini）
# 添加了 inotify swoole unzip
# 不推荐使用，phpunit 运行失败
# docker build -f phalcon-5.8.0-s.Dockerfile  -t authus/phalcon:5.8.0-s .