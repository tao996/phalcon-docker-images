...
RUN set -xe && \
        ...
        mv /usr/local/etc/php/conf.d/docker-php-ext-xdebug.ini /usr/local/etc/php/conf.d/xdebug.ini.disabled && \
        mv /usr/local/etc/php/conf.d/docker-php-ext-phalcon.ini /usr/local/etc/php/conf.d/phalcon.ini.disabled
        


WORKDIR /var/www

# docker build -f phalcon-5.5.Dockerfile -t authus/phalcon:5.5 .
